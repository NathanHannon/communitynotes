...
from note_writer.misleading_tags import get_misleading_why_tags


def _get_prompt_for_note_writing(
    post_with_context_description: str, search_results: str
):
    return f"""
    I have a post that I need to write a Community Note for.
    Below will be a post on X, and live search results from the web. \
...
If you are writing a note, don't preface it with anything like "Community Note:". Just write the note.
    ...
    Post: {post_with_context_description}
    """


def _get_post_with_context_description_for_prompt(
...
def _check_for_unsupported_media_in_post_with_context(
    ...
    if _check_for_unsupported_media(post_with_context.post):
        return True
    if post_with_context.quoted_post and _check_for_unsupported_media(
        post_with_context.quoted_post
    ):
        return True
    if post_with_context.in_reply_to_post and _check_for_unsupported_media(
        post_with_context.in_reply_to_post
    ):
        return True
    return False
...
def research_post_and_write_note(
    ...
    post_with_context_description = _get_post_with_context_description_for_prompt(
        post_with_context
    )

    note_prompt = _get_prompt_for_note_writing(post_with_context_description, "")
